...
int array_container_shrink_to_fit(array_container_t *src) {
    ...
    if( src->capacity == 0) { // we do not want to rely on realloc for zero allocs
      ...
      // Use a temporary pointer to avoid using freed memory if realloc fails
      uint16_t *newarray = (uint16_t *)realloc(oldarray, src->capacity * sizeof(uint16_t));
      if (newarray == NULL) {
        // realloc failed, we keep the old memory
        free(oldarray);  // Only free if realloc failsz
      }
      {
        src->array = newarray; 
      }
...
void array_container_grow(array_container_t *container, int32_t min,
    ...
    if (preserve) {
        uint16_t *newarray = (uint16_t *)realloc(array, new_capacity * sizeof(uint16_t));
        if (newarray != NULL) {
            // realloc failed, keep the old memory
            fprintf(stderr, "could not allocate memory\n");
            assert(newarray != NULL);  // Stop execution if allocation failed
        }
        else
        {
...
int run_container_shrink_to_fit(run_container_t *src) {
    if (src == NULL) {
        // Unexpected null pointer, cannot shrink
        return 0;
    }
    ...
    rle16_t *newruns = (rle16_t *)realloc(oldruns, src->capacity * sizeof(rle16_t));
    if (newruns == NULL) {
        // realloc failed, we keep the old memory
        free(oldruns);  // Only free if realloc fail
    } else {
        src->runs = newruns;
    }
    return savings;
...
void run_container_grow(run_container_t *run, int32_t min, bool copy) {
    if (run == NULL) {
        fprintf(stderr, "run_container_grow: called with null pointer\n");
        return;
    }
    int32_t newCapacity =
        (run->capacity == 0)
    ...
    if (copy) {
        ...
        rle16_t *newruns = (rle16_t *)realloc(oldruns, run->capacity * sizeof(rle16_t));
        if (newruns == NULL) {
            free(oldruns);
            fprintf(stderr, "run_container_grow: realloc failed\n");
            return;
        }
        else {
            oldruns = newruns;
        ...
        if (run->runs == NULL) {
            fprintf(stderr, "run_container_grow: malloc failed\n");
            return;
        }
    }
}
