...
void test_one() {
...

    {
        ...
        // 初始化加密上下文
        if (EVP_EncryptInit_ex(ctx, EVP_aes_256_cbc(), nullptr, (const uint8_t*)key.c_str(), (const uint8_t*)iv.c_str()) != 1) {
        ...
        // 分配输出缓冲区并处理加密
        std::string encode;
        encode.resize(str.size() + EVP_CIPHER_block_size(EVP_aes_256_cbc())); // 确保缓冲区足够大
        int olen = 0;
        if (EVP_EncryptUpdate(ctx, (uint8_t*)&encode[0], &olen, (const uint8_t*)str.c_str(), str.length()) != 1) 
        {
        ...
        int llen = 0;
        if (EVP_EncryptFinal_ex(ctx, (uint8_t*)(&encode[0] + olen), &llen) != 1) 
        {
